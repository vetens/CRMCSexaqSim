...
for i in range(0,args.nfiles):
	...
	file.write("(eval `scram unsetenv -sh`; gfal-mkdir " + args.gfal_prefix + "/store/user/" + args.user + "/crmc_Sexaq/crmc/Sexaquark_13TeV_trial_"+ args.trialname + " >/dev/null)"+"\n")
	#sometimes the copy failed the first time so therefore I put it twice ...
	file.write("(eval `scram unsetenv -sh`; gfal-copy " + args.workdir + "/CustomGenerator/Custom/crmc_Sexaq_incl_installed/SexaquarkProduction/OutputRootFile/OutputRootFile_with_Vtx_smearing2/crmc_Sexaq_" + str(i) + ".root " + args.gfal_prefix + "/store/user/" + args.user + "/crmc_Sexaq/crmc/Sexaquark_13TeV_trial_" + args.trialname + "/crmc_Sexaq_" + str(i) + ".root)" + "\n")
	file.write("(eval `scram unsetenv -sh`; gfal-copy " + args.workdir + "/CustomGenerator/Custom/crmc_Sexaq_incl_installed/SexaquarkProduction/OutputRootFile/OutputRootFile_with_Vtx_smearing2/crmc_Sexaq_" + str(i) + ".root " + args.gfal_prefix + "/store/user/" + args.user + "/crmc_Sexaq/crmc/Sexaquark_13TeV_trial_" + args.trialname + "/crmc_Sexaq_" + str(i) + ".root)" + "\n")
...
file2 = open("condor/condor_multiple.cfg", "w")
...
file2.write("Log        = " + args.workdir + "/CustomGenerator/Custom/crmc_Sexaq_incl_installed/CRMCSexaqSim/condor/$(ProcID).log\n")
file2.write("Output     = " + args.workdir + "/CustomGenerator/Custom/crmc_Sexaq_incl_installed/CRMCSexaqSim/condor/$(ProcID).out\n")
file2.write("Error      = " + args.workdir + "/CustomGenerator/Custom/crmc_Sexaq_incl_installed/CRMCSexaqSim/condor/$(ProcID).error\n")
